    ...
    switch (url.indexOf("?")) {
        ...
        default:
            ...
            for (let i = 0; i < hash.length; i++) {
                if (hash[i].indexOf("=") != -1) {
                    items[hash[i].split('=')[0]] = hash[i].split('=')[1];
                }
            }
            onloadAJax(items);
            break;
...
function general_string(items) {
    ...
    return `<main style='position:absolute;flex-grow:1;top:60px;right:32px;left:32px;height:auto;'><div style='margin:0 auto;width:95%;max-width:95%;flex-basis:95%;'><div style='margin:0 auto;padding:0px 32px;max-width:770px;flex-basis:100%;'>${container_row(auto_item(subject) + no_item(name))}${note}${general_textarea("textarea", "SIGN OUT REASON")}${container_row(auto_item("") + no_item(general_button("surebutn", "核准")) + no_item(general_button("backbutn", "退回")))}</div></div></main>`;
}
...
function onloadAJax(items) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            $("body").append(error_string("this database has problem"));
...
function signAJax(items, url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            switch (data.status) {
                case "istrue":
                    $("body").html(error_string("this form sign successfully"));
                    break;
                default:
                    $("body").html(error_string("this form has problem"));
                    break;
            }
        },
        error: function (xhr, status, error) {
